...
Esperado: `All tests passed!`, 12 testes, e `analyze` limpo.
...
- Produces:
  - `class LinhaResponsiva { const LinhaResponsiva({required Key chave, required List<String> valores, VoidCallback? aoTocar, VoidCallback? aoExcluir}); }`
...
Esperado: `All tests passed!`, 10 testes, e `analyze` limpo.
...
Esperado: `All tests passed!`, 14 testes, e `analyze` limpo.
