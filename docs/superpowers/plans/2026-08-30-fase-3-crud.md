...
Future<RepositorioGastosFake> montar(
  ...
  addTearDown(container.dispose);
  container.read(mesSelecionadoProvider.notifier).irPara(const MesRef(2026, 8));
  // Assina antes de montar para que potes e casa ja tenham emitido. NAO
  // use `await Future.delayed(...)` aqui: dentro de testWidgets o relogio e
  // falso e so avanca com pump(), entao um delay puro trava o teste inteiro
  // ("did not complete"). Os fakes usam Stream.multi justamente para emitir
  // o valor corrente de forma sincrona no listen.
...
- [x] **Step 5: Verificar no app real, no Windows**
...
- [x] **Step 6: Verificar a sincronização no Android**
