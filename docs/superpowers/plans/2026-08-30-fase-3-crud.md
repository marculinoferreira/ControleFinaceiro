...
class TelaGastos extends ConsumerWidget {
  ...
  Widget _tabela(
    ...
    String nomeMembro(String id) =>
        membros.where((m) => m.id == id).map((m) => m.nome).firstOrNull ?? id;
    String nomePote(String id) =>
        potes.where((p) => p.id == id).map((p) => p.nome).firstOrNull ?? id;
...
class TelaParcelas extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return abertas.when(
      ...
      data: (compras) {
        String nomeMembro(String id) => membros
                .where((m) => m.id == id)
                .map((m) => m.nome)
                .firstOrNull ??
            id;
        String nomePote(String id) =>
            potes.where((p) => p.id == id).map((p) => p.nome).firstOrNull ??
            id;
